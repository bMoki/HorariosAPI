...
public class ExceptionHandler {
    ...
    @org.springframework.web.bind.annotation.ExceptionHandler(WrongCollumnsException.class)
    public ResponseEntity<StandardError> wrongCollumnsError(WrongCollumnsException e, HttpServletRequest request) {
        StandardError err = new StandardError(
                Instant.now(),
                HttpStatus.BAD_REQUEST.value(),
                "Collumns not matched",
                e.getMessage(),
                request.getRequestURI()
        );
    ...
    @org.springframework.web.bind.annotation.ExceptionHandler(LastUserException.class)
    public ResponseEntity<StandardError> lastUserException(LastUserException e, HttpServletRequest request) {
        StandardError err = new StandardError(
                Instant.now(),
                HttpStatus.BAD_REQUEST.value(),
                "last user in database",
                e.getMessage(),
                request.getRequestURI()
        );
